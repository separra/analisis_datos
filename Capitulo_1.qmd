...
Este documento contiene los resultados del experimento realizado con tres accesiones de *Capsicum spp.* crecidas en el muncipio de Palmira, departamento del Valle del Cauca, Colombia. En el cual se evaluaron las características físicas y químicas de los frutos obtenidos, empleando diferentes parámetros.
...
Se presentan, paso a paso, los métodos para los análisis realizados y las interpretaciones de los resultados obtenidos. De esta forma se busca que el lector pueda comprender y aplicar métodos de **estadística descriptiva e inferencial**
...
Para iniciar con el análisis, es necesario cargar la información contenida en la base de datos. Generalmente los datos se almacenan en archivos .txt, .csv o de Excel. En este capítulo, los datos usados corresponden a las características físicas y químicas de los frutos obtenidos de las accesiones de Capsicum. En el análisis se emplearon cinco repteiciones por cada accesoión. La base de datos que se presenta a continuación contiene todas las observaciones de las variables medidas. Para cargar el archivo con los datos se emplea la función `read.delim()`, la cual permite cargar un archivo delimitado por tabulaciones.
...
#| output: false
#| code-fold: true
caro.cauca$acce <- as.factor (caro.cauca$acce)
caro.cauca$espe <- as.factor (caro.cauca$espe)
caro.cauca$acar <- as.numeric (caro.cauca$acar)
caro.cauca$bcar <- as.numeric (caro.cauca$bcar)
caro.cauca$zea <- as.numeric (caro.cauca$zea)
caro.cauca$bcrip <- as.numeric (caro.cauca$bcrip)
caro.cauca$lico <- as.numeric (caro.cauca$lico)
caro.cauca$provitA <- as.numeric (caro.cauca$provitA)
caro.cauca$brix <- as.numeric (caro.cauca$brix)
caro.cauca$ph <- as.numeric (caro.cauca$ph)
caro.cauca$hum <- as.numeric (caro.cauca$hum)
caro.cauca$L <- as.numeric (caro.cauca$L)
caro.cauca$a <- as.numeric (caro.cauca$a)
caro.cauca$b <- as.numeric (caro.cauca$b)
caro.cauca$C <- as.numeric (caro.cauca$C)
caro.cauca$h <- as.numeric (caro.cauca$h)
```
...
Para el cálculo de los promedios se empleó la función `smmarise_all ()`. Previamente el conjunto de datos se agrupó de acuerdo con las accesiones evaluadas empleando la función `group_by()`

```{r}
#| label: Cálculo de promedios por accesión
#| warning: false 
#| message: false
#| tbl-cap: Tabla 1. Promedios por accesión
#| code-fold: true
proms.cauca <- caro.cauca %>% group_by (acce, espe) %>% 
  summarise_all(funs(mean(.), se=sd(.)/sqrt(n())))

datatable(proms.cauca, rownames=FALSE, class = 'cell-border stripe')
```

La tabla presenta los promedios y desviación estandar para las variables numéricas evaluadas agrupando las observaciones por las accesiones. Estos valores serán empleados para la elaboración de las figuras que componen el análisis.
